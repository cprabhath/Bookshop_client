  ...
  "group pointer-events-auto relative flex w-full items-center space-x-2 overflow-hidden rounded-md border p-4 pr-6 shadow-lg transition-all data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--radix-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--radix-toast-swipe-move-x)] data-[swipe=move]:transition-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out data-[state=closed]:fade-out-80 data-[state=closed]:slide-out-to-left-full data-[state=open]:slide-in-from-top-full data-[state=open]:sm:slide-in-from-bottom-full",
  {
    variants: {
      variant: {
        ...
        success: "bg-green-700 text-white border-none",
        warning: "bg-yellow-700 text-white border-none",
        info: "bg-sky-700 text-white border-none",
      },
    },
    defaultVariants: {
      variant: "default",
...
>(({ className, variant, ...props }, ref) => {
  // Define icons based on the variant
  const icon = React.useMemo(() => {
    switch (variant) {
      case "warning":
        return <ExclamationTriangleIcon className="h-7 w-7 text-white" />;
      case "info":
        return <InfoCircledIcon className="h-7 w-7 text-white" />;
      case "success":
        return <CheckCircledIcon className="h-7 w-7 text-white" />;
      case "destructive":
        return <Cross2Icon className="h-7 w-7 text-white" />;
      default:
        return null;
    }
  }, [variant]);
  ...
  return (
    <ToastPrimitives.Root
      ...
      {...props}
    >
      {/* Render the icon at the beginning of the toast */}
